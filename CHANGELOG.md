...
## [Unreleased]
- Added comprehensive configuration file support. Users can now encode all command-line options in a configuration file (INI or JSON format) instead of typing them every time. The tool automatically searches for `./dangling-dns.conf` or `~/.dangling-dns.conf` and loads them if found. Alternatively, specify a custom config file with `--config path/to/config.conf`. Example configuration files (`dangling-dns.example.conf` and `dangling-dns.example.json`) are provided. Command-line arguments always take precedence over config file values, allowing users to override specific settings per-run while maintaining defaults in the config file.
- Added special `previous` value for `--compare-to` option. Use `--compare-to previous` to automatically find and compare against the most recent `records_YYYY-MM-DD.json` file based on modification time. Eliminates the need to manually specify the previous results file each time.
